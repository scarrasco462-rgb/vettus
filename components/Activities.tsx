    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-10">
      ...
      {viewMode === 'calendar' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white rounded-[1.5rem] lg:rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden flex flex-col">
            <div className="navy-gradient p-5 lg:p-8 flex items-center justify-between text-white">
              <h2 className="text-lg lg:text-xl font-bold uppercase tracking-widest flex items-center">
              ...
              <div className="flex items-center space-x-2">
                ...
                <button onClick={() => setCurrentDate(new Date())} className="px-3 lg:px-4 py-1.5 bg-white/10 hover:bg-white/20 rounded-lg text-[9px] lg:text-[10px] font-bold uppercase tracking-widest transition-colors">Hoje</button>
            ...
            <div className="p-4 lg:p-8 grid grid-cols-7 gap-px bg-slate-100">
              {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(day => (
                <div key={day} className="bg-white py-3 lg:py-4 text-center">
                  <span className="text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-slate-400">{day}</span>
                </div>
              ))}
              ...
              {calendarDays.map((slot, idx) => {
                ...
                return (
                  <div key={idx} onClick={() => { if(slot.day) { setSelectedDay(slot.day); setIsModalOpen(true); } }} className={`bg-white h-24 lg:h-32 p-2 lg:p-3 relative group transition-all cursor-pointer hover:bg-slate-50 ${!slot.current ? 'bg-slate-50/50' : ''}`}>
                    <span className={`text-[10px] lg:text-xs font-bold ${isToday ? 'bg-[#d4a853] text-white w-5 h-5 lg:w-6 lg:h-6 rounded-full flex items-center justify-center shadow-md' : slot.current ? 'text-slate-900' : 'text-slate-300'}`}>{slot.day}</span>
                    <div className="mt-1 lg:mt-2 space-y-0.5 lg:space-y-1 overflow-hidden">
                      {dayActs.slice(0, 3).map(act => (
                        <div key={act.id} onClick={(e) => handleActivityClick(act, e)} className="bg-slate-100 rounded-md p-0.5 lg:p-1 border-l-2 border-[#d4a853] overflow-hidden cursor-pointer hover:bg-[#d4a853]/10 transition-colors">
                          <p className="text-[8px] lg:text-[9px] font-bold text-slate-700 truncate">{act.description}</p>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="bg-[#0a1120] p-6 lg:p-10 rounded-[2rem] lg:rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
             <div className="absolute top-0 right-0 p-8 opacity-5"><Bell className="w-24 h-24 lg:w-32 lg:h-32" /></div>
             <h3 className="text-base lg:text-lg font-black uppercase tracking-widest text-[#d4a853] mb-4 lg:mb-6">Métricas de Execução</h3>
             <div className="space-y-4 lg:space-y-6">
                <div className="bg-white/5 p-4 rounded-xl lg:rounded-2xl border border-white/10">
                   <p className="text-[10px] font-black uppercase text-slate-500 mb-1">Total na Timeline</p>
                   <p className="text-xl lg:text-2xl font-black">{activities.length}</p>
                </div>
                <div className="bg-white/5 p-4 rounded-xl lg:rounded-2xl border border-white/10">
                   <p className="text-[10px] font-black uppercase text-slate-500 mb-1">Visitas Realizadas</p>
                   <p className="text-xl lg:text-2xl font-black text-emerald-500">{activities.filter(a => a.type === 'Viewing').length}</p>
                </div>
             </div>
          </div>
        </div>
      ) : (
        <div className="max-w-4xl mx-auto space-y-6 lg:space-y-8 pb-20 no-scrollbar">
          <div className="relative space-y-6 lg:space-y-10 before:absolute before:inset-0 before:ml-4 lg:before:ml-5 before:-translate-x-px before:h-full before:w-1 before:bg-slate-200">
            {filteredActivities.map((activity) => (
              <div key={activity.id} className="relative flex items-start group">
                <div className="flex items-center justify-center w-8 h-8 lg:w-10 lg:h-10 rounded-full border-4 border-slate-50 bg-[#0a1120] text-[#d4a853] shadow-xl z-10 group-hover:scale-125 transition-transform">
                  {getActivityIcon(activity.type)}
                </div>
                <div onClick={(e) => handleActivityClick(activity, e)} className="ml-6 lg:ml-10 flex-1 p-5 lg:p-8 rounded-[1.5rem] lg:rounded-[2.5rem] bg-white border border-slate-100 shadow-sm hover:shadow-2xl hover:border-[#d4a853]/30 transition-all cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex items-center justify-between mb-2 lg:mb-4">
                    <div className="flex items-center space-x-2 lg:space-x-3">
                       <span className={`px-2 lg:px-3 py-0.5 lg:py-1 rounded-full text-[8px] lg:text-[9px] font-black uppercase tracking-widest border ${
                         activity.type === 'Viewing' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-500 border-slate-100'
                       }`}>{activity.type}</span>
                       <span className="text-[8px] lg:text-[10px] font-bold text-slate-400">POR {activity.brokerName?.toUpperCase()}</span>
                    </div>
                    <div className="flex items-center text-[#d4a853] font-black text-[9px] lg:text-[10px] uppercase">
                       <Clock className="w-3 h-3 lg:w-3.5 lg:h-3.5 mr-1" />
                       {activity.time}
                    </div>
                  </div>
                  <h3 className="text-sm lg:text-lg font-black text-slate-900 mb-1 lg:mb-2">{activity.description}</h3>
                  <div className="flex items-center space-x-3 lg:space-x-4">
                     <div className="flex items-center text-[10px] lg:text-xs font-bold text-slate-600">
                        <UserIcon className="w-3 h-3 lg:w-3.5 lg:h-3.5 mr-1 lg:mr-1.5 text-slate-400" />
                        {activity.clientName}
                     </div>
                     <div className="flex items-center text-[10px] lg:text-xs font-bold text-slate-400">
                        <CalendarIcon className="w-3 h-3 lg:w-3.5 lg:h-3.5 mr-1 lg:mr-1.5 text-slate-400" />
